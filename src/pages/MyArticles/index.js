...
import {LoadingArea, Loading} from './styles';
...
export default function MyArticles() {
  ...
  const getMyArticles = async (page, limit) => {
    try {
      ...
      const response = await api.get(
        '/myarticles/' + idAutor + '?page=' + page + '&limit=' + limit,
        {
          headers: {
  ...
  const editRow = (rowMap, idArticle) => {
    {
      setLoading(true);
  ...
  const deleteRow = async (rowMap, idArticle) => {
    setLoading(true);
    const token = await AsyncStorage.getItem('@token');
    ...
    await api
      ...
      .catch((err) => {
        if (err.response) {
          ...
          Alert.alert(
            '',
            'Artigo não foi deletado com sucesso, tente mais tarde!',
          );
        }
        setLoading(false);
      });
  ...
  const renderItem = (data, rowMap) => {
    ...
    return (
      <VisibleItem
        ...
        editRow={() => editRow(rowMap, data.item.id)}
      />
    );
  };
